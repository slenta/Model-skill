...
class calculate_leadyear(object):
    ...
    def __getitem__(self, start_year, end_year):
        ...
        hist = get_variable(
            ...
            lead_year=lead_year,
            name=cfg.hist_name,
            ensemble_members=cfg.ensemble_member_hist,
            start_year=start_year,
            end_year=end_year,
            start_month="01",
    ...
    def calculate_lead_corr(self):
        ...
        if cfg.region == "global":
            obs, hind, hist, res_obs, res_hind = np.zeros(
                shape=(
                    5,
                    self.end_year - self.start_year,
                    cfg.lonlats[3] - cfg.lonlats[2],
                    cfg.lonlats[1] - cfg.lonlats[0],
                )
            )
        else:
            obs, hind, hist, res_obs, res_hind = np.zeros(
                shape=(
                    ...
                    self.end_year - self.start_year,
                    cfg.lonlats[3] - cfg.lonlats[2],
                    cfg.lonlats[1] - cfg.lonlats[0] + 1,
                )
            )

        if type(self.lead_year) == int:
            for i in range(
                self.start_year - self.lead_year, self.end_year - self.lead_year
            ):
                (
                    obs[i - self.start_year],
                    hind[i - self.start_year],
                    hist[i - self.start_year],
                ) = self.__getitem__(i, i + cfg.hind_length)
        else:
        ...
        f_res = h5.File(
            f"{cfg.residual_path}_{str(self.start_year)}_{str(lead_year)}.hdf5", "r"
        )
    ...
    def save_lead_corr(self):
        ...
        f = h5.File(
            f"{cfg.tmp_path}correlation/correlation_{self.start_year}_{self.end_year}_{lead_year}.hdf5",
            "w",
        )
        dset1 = f.create_dataset(
            "hind_corr", (n[0], n[1]), dtype="float32", data=hind_corr
        )
        dset2 = f.create_dataset(
            "res_hind_corr", (n[0], n[1]), dtype="float32", data=res_hind_corr
        )
        dset3 = f.create_dataset(
            "hist_corr", (n[0], n[1]), dtype="float32", data=hist_corr
        )
        f.close()
...
class ly_series(object):
    ...
    def ly_series(self):
        ...
        ax.grid()
        fig.suptitle(
            "Hindcast Correlation by Lead Year: {} -- {}".format(
                str(cfg.start_year), str(cfg.end_year)
            )
        )
        plt.legend()
        plt.savefig(
            "{}leadyear_timeseries_{}_{}.pdf".format(
                cfg.plot_path, str(cfg.start_year), str(cfg.end_year)
            )
        )
        plt.show()
